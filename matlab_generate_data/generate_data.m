...
while(fcnt <=loopBound)
    ...
    for ff=1:imuSamplesPerGPS
        ...
        fcnt = fcnt + 1;
        data.IMU.t = cat(2, data.IMU.t, time);
        data.IMU.acc = cat(2, data.IMU.acc, accel');
    ...
    data.GPS.t = cat(2, data.GPS.t, time);
    data.GPS.pos_NED = cat(2, data.GPS.pos_NED, lla');
    data.GPS.vel = cat(2, data.GPS.vel, gpsvel');
