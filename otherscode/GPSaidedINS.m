...
% out_data.x_deltau_h        Estimated IMU biases [accelerometers; gyroscopes]
...
u_measure_raw=[in_data.IMU.acc;in_data.IMU.gyro];
t=in_data.IMU.t;
...
p_v_theta=init_navigation_state(u_measure_raw,settings);

% Initialize the sensor bias estimate
acc_gro_bias=zeros(6,1);
...
N=size(u_measure_raw,2);
out_data.x_h=zeros(10,N);
out_data.x_h(:,1)=p_v_theta;
out_data.diag_P=zeros(15,N);
out_data.diag_P(:,1)=diag(P);
out_data.x_deltau_h=zeros(6,N);
...
for k=2:N
    ...
    u_meansure_with_bias=u_measure_raw(:,k)+acc_gro_bias;
    
    
    % Update the INS navigation state
    p_v_theta=Nav_eq(p_v_theta,u_meansure_with_bias,Ts,settings.gravity);
    
    % Get state space model matrices
    [F,G]=state_space_model(p_v_theta,u_meansure_with_bias,Ts);
    ...
    Rn2p = get_Rb2p()*q2dcm(p_v_theta(7:10))';
    H1 = [eye(3) zeros(3,12)];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [F,G]=state_space_model(p_v_theta,u_meansure_with_bias,Ts)

% Convert quaternion to DCM
Rb2t=q2dcm(p_v_theta(7:10));

% Transform measured force to force in
% the tangent plane coordinate system.
f_t=Rb2t*u_meansure_with_bias(1:3);
